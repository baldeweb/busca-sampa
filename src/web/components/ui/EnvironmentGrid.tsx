...
const EnvironmentGrid: React.FC<EnvironmentGridProps> = ({ environments, selectedEnv, onSelect, onViewMore, title, showViewMore = true, containerClassName, contentPaddingClassName, gridClassName }) => {
  ...
  const gridClass = gridClassName || "grid grid-cols-3 min-[790px]:grid-cols-5 gap-2 text-xs";
  return (
    <div className={containerClassName || "bg-[#FFFFFF] text-black pb-4"}>
      ...
      <div className={`${gridClass} ${paddingClassName}`}>
        ...
        <button
          ...
          className={`btn-square btn-red-outline btn-hover-red font-semibold uppercase rounded-md px-4 py-4 leading-tight transition-colors border shadow-sm ${
            selectedEnv === null
        ...
        {environments.slice(0, 8).map((env, idx) => (
          <button
            ...
            className={`btn-square btn-hover-gray font-semibold uppercase rounded-md px-4 py-4 leading-tight transition-colors border shadow-sm ${
          ...
          >
            <span
              style={{
                ...
                lineHeight: '1.1',
                maxHeight: '2.2em', // Limite máximo para 2 linhas, mas não força altura
              }}
              title={env.label}
            >
              {env.label}
        ...
        {showViewMore && environments.length > 4 && (
          <button
            type="button"
            onClick={onViewMore}
            className={`btn-square-dark btn-hover-red py-4 font-semibold text-xs rounded-md ${
